...
new_login = '''    fun login(context: Context, email: String, existingSessionToken: String? = null, onSessionCreated: ((String) -> Unit)? = null) {
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                ...
                e.printStackTrace()
                android.util.Log.e("AuthViewModel", "Ping failed", e)
                val stackTrace = android.util.Log.getStackTraceString(e)
                val mainThread = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Main) {
                    authState = AuthState.Error("Network error pinging Google: ${e.message}\\n\\nDetails:\\n${stackTrace.take(500)}")
                }
                return@launch
            }
